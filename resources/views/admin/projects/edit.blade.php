    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4 border-bottom border-light">
        <div>
            <h1 class="h2 fw-bold text-dark">
                <i class="fas {{ isset($project) ? 'fa-edit' : 'fa-plus-circle' }} me-2 text-primary"></i>
                {{ isset($project) ? 'Edit Project' : 'Add New Project' }}
            </h1>
            <p class="text-secondary small mb-0">{{ isset($project) ? 'Update project details' : 'Create a new project' }}</p>
        </div>
                ...
                <div class="col-lg-8">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('category') is-invalid @enderror" 
                            ...
                            <option value="ai" {{ old('category', $project->category ?? '') == 'ai' ? 'selected' : '' }}>AI/ML</option>
                            <option value="blockchain" {{ old('category', $project->category ?? '') == 'blockchain' ? 'selected' : '' }}>Blockchain</option>
                            ...
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="status-switch">
                                    <input type="checkbox" 
                                           ...
                                           {{ old('status', $project->status ?? 0) ? 'checked' : '' }}>
                                ...
                                <div class="status-indicator">
                                    <span class="badge {{ old('status', $project->status ?? 0) ? 'bg-success' : 'bg-secondary' }} rounded-pill px-3 py-2" id="statusBadge">
                                        <i class="fas {{ old('status', $project->status ?? 0) ? 'fa-check-circle' : 'fa-times-circle' }} me-1"></i>
                                        <span id="statusLabel">{{ old('status', $project->status ?? 0) ? 'Active' : 'Inactive' }}</span>
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="technologies-wrapper">
                            @php
                                $technologies = old('technologies', isset($project) ? json_decode($project->technologies, true) ?? [] : []);
                            @endphp
                ...
                <div class="col-lg-4">
                        ...
                        @if(isset($project) && $project->image)
                            <div class="current-image mb-2">
                                <img src="{{ asset('images/' . $project->image) }}" 
                                     ...
                                     style="max-height: 200px; width: 100%; object-fit: cover; border: 2px solid rgba(0,0,0,0.1);">
                                <div class="form-check mt-2">
                                    <input class="form-check-input" type="checkbox" name="remove_image" id="remove_image" value="1">
                                    <label class="form-check-label text-secondary" for="remove_image">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('client_review') is-invalid @enderror" 
                                  ...
                                  name="client_review" 
                                  rows="4" 
                                  placeholder="Enter client review or testimonial">{{ old('client_review', $project->client_review ?? '') }}</textarea>
                        @error('client_review')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
        ...
        if (removeImageCheckbox) {
            removeImageCheckbox.addEventListener('change', function() {
                ...
                if (this.checked) {
                    ...
                    // Clear file input if image is removed
                    if (imageInput) {
                        imageInput.value = '';
